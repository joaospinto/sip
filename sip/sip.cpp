...
auto unscaled_max_difference(const double *lhs, const double *rhs,
                             const double *scaling, const int size) -> double {
  double result = 0.0;
  for (int i = 0; i < size; ++i) {
    const double difference = lhs[i] - rhs[i];
    const double unscaled_difference =
        scaling == nullptr ? difference : difference / scaling[i];
    result = std::max(result, std::fabs(unscaled_difference));
  }
  return result;
}
...
auto fixed_dual_merit_slope(const Input &input, const Workspace &workspace,
                            const bool use_proximal_qp, const double mu,
  ...
  double s_slope;
  if (use_proximal_qp) {
    s_slope = weighted_dot(gps, workspace.penalties.z, ds, s_dim);
    for (int i = 0; i < s_dim; ++i) {
      s_slope += (workspace.vars.z[i] - mu / workspace.vars.s[i]) * ds[i];
    }
  } else {
    s_slope = dot(workspace.nrhs.s, ds, s_dim) +
...
auto compute_search_direction(const Input &input, const Settings &settings,
  ...
  for (int attempt = 0; attempt < settings.regularization.max_attempts;
       ++attempt) {
    const double x_regularization = use_proximal_qp ? proximal_rho : psi;
    factorization_ok =
    ...
    if (use_proximal_qp) {
      numerical_regularization =
          numerical_regularization == 0.0
              ? settings.proximal_qp.first_factorization_regularization
              : settings.regularization.increase_factor *
                    numerical_regularization;
  ...
  const auto solve_direction = [&](const double target_mu,
    ...
    if (use_proximal_qp) {
      ...
      for (int i = 0; i < y_dim; ++i) {
        by[i] += proximal_delta *
                 (workspace.proximal_centers.y[i] - workspace.vars.y[i]);
      }
    }
    for (int i = 0; i < s_dim; ++i) {
    ...
    const auto compute_refinement_residual = [&]() {
      ...
      const double x_regularization = use_proximal_qp ? proximal_rho : psi;
      input.add_Kx_to_y(w, x_regularization, r2, r3, vx, vy, vz, res_x, res_y,
                        res_z);
    };
    compute_refinement_residual();
    double previous_refinement_error = max_abs_or_inf(residual, dim_3x3);
    constexpr double kMaxRelativeRefinementCorrection = 10.0;
    constexpr double kMinimumRefinementImprovement = 5.0;
    ...
    for (int j = 0; j < num_iterative_refinement_steps; ++j) {
      ...
      if (use_proximal_qp &&
          max_abs_or_inf(u, dim_3x3) >
              kMaxRelativeRefinementCorrection *
                  std::max(1.0, max_abs_or_inf(v, dim_3x3))) {
        break;
      }
      for (int i = 0; i < dim_3x3; ++i) {
        v[i] -= u[i];
      ...
      if (improvement < kMinimumRefinementImprovement) {
        if (improvement <= 1.0) {
          for (int i = 0; i < dim_3x3; ++i) {
            v[i] += u[i];
          }
  ...
  if (settings.proximal_qp.enabled && s_dim > 0) {
    ...
    stationarity_merit_slope =
        fixed_dual_merit_slope(input, workspace, true, mu, dx, ds).total;
    ...
    solve_direction(mu, nullptr, nullptr);
    stationarity_merit_slope =
        fixed_dual_merit_slope(input, workspace, settings.proximal_qp.enabled,
                               mu, dx, ds)
            .total;
  }

  const auto fixed_dual_ms = fixed_dual_merit_slope(
      input, workspace, settings.proximal_qp.enabled, mu, dx, ds);
  ...
  if (settings.logging.print_search_direction_logs) {
    ...
    const double x_regularization = use_proximal_qp ? proximal_rho : psi;
    input.add_Kx_to_y(w, x_regularization, r2, r3, dx, dy, dz, res_x, res_y,
                      res_z);
  ...
  return std::make_tuple(
      true, dx, ds, dy, dz, stationarity_merit_slope, fixed_dual_ms.total,
...
auto do_line_search(const Input &input, const Settings &settings,
  ...
  do {
    ...
    if (settings.proximal_qp.enabled) {
      ...
      accepted_without_filter =
          skip_line_search || merit_delta < settings.line_search.armijo_factor *
                                                merit_slope * alpha;
    } else {
      accepted_without_filter =
          merit_slope >
  ...
  const double dual_step =
      settings.proximal_qp.enabled ? std::min(alpha, alpha_z_max) : alpha;
...
auto check_settings(const Settings &settings) -> bool {
  ...
  if (settings.max_iterations < 0 || settings.line_search.max_iterations < 0 ||
      ...
      settings.proximal_qp
              .max_center_update_rejections_before_regularization_refinement <=
          0 ||
      settings.proximal_qp.max_consecutive_center_update_rejections <= 0 ||
  ...
  if (!is_finite_positive(settings.proximal_qp.initial_primal_regularization) ||
      !is_finite_positive(settings.proximal_qp.initial_dual_regularization) ||
      !is_finite_positive(
          settings.proximal_qp.initial_minimum_regularization) ||
      !is_finite_positive(settings.proximal_qp.minimum_regularization) ||
      settings.proximal_qp.minimum_regularization >
          settings.proximal_qp.initial_minimum_regularization ||
      settings.proximal_qp.initial_minimum_regularization >
          settings.proximal_qp.initial_primal_regularization ||
      settings.proximal_qp.initial_minimum_regularization >
          settings.proximal_qp.initial_dual_regularization ||
      !is_finite_positive(
          settings.proximal_qp.first_factorization_regularization)) {
    return false;
  }
...
auto initialize_proximal_qp(const Input &input, const Settings &settings,
                            Workspace &workspace) -> bool {
  const int x_dim = input.dimensions.x_dim;
  const int s_dim = input.dimensions.s_dim;
  const int y_dim = input.dimensions.y_dim;
  ...
  for (int i = 0; i < y_dim; ++i) {
    r2[i] = settings.proximal_qp.initial_dual_regularization;
  }
  for (int i = 0; i < s_dim; ++i) {
    r3[i] = settings.proximal_qp.initial_dual_regularization;
  }
...
auto solve(const Input &input, const Settings &settings, Workspace &workspace)
  ...
  double proximal_rho = settings.proximal_qp.initial_primal_regularization;
  double proximal_delta = settings.proximal_qp.initial_dual_regularization;
  ...
  constexpr double kRejectedCenterRegularizationReductionScale = 0.666;
  constexpr int kCenterUpdateStartupIterations = 5;
  int primal_center_update_rejections_before_regularization_refinement = 0;
  int dual_center_update_rejections_before_regularization_refinement = 0;
  ...
  for (int iteration = 0; iteration < settings.max_iterations; ++iteration) {
      ...
      const double primal_proximal_residual =
          ...
          std::max(unscaled_max_difference(
                   ...
                   unscaled_max_difference(
                       ...
                       input.residual_scaling.inequality, s_dim));
      const int max_rejections =
          settings.proximal_qp
              .max_center_update_rejections_before_regularization_refinement;
      if (((primal_center_update_rejections_before_regularization_refinement >=
                max_rejections &&
        ...
        proximal_regularization_limit =
            settings.proximal_qp.minimum_regularization;
        primal_center_update_rejections_before_regularization_refinement = 0;
        dual_center_update_rejections_before_regularization_refinement = 0;
      }
    }
    ...
    const auto [factorization_ok, dx, ds, dy, dz, stationarity_merit_slope,
        ...
        compute_search_direction(input, settings, mu, psi, proximal_rho,
                                 proximal_delta, tau, workspace);
    ...
    if (settings.line_search.skip_line_search) {
      alpha = alpha_s_max;
      update_next_primal_vars(input, tau, workspace, alpha, true, true);
      update_next_dual_vars(input, settings, tau, workspace,
                            settings.proximal_qp.enabled ? alpha_z_max : alpha);
      ls_succeeded = true;
      m0 = 0.0;
    } else {
    ...
    if (settings.proximal_qp.enabled) {
      ...
      if (new_dual_residual < kResidualReductionFactor * dual_residual ||
        ...
        if (iteration < kCenterUpdateStartupIterations ||
            new_dual_proximal_residual < kProximalResidualThreshold) {
          proximal_rho =
              std::max(proximal_regularization_limit,
      ...
      if (new_primal_residual <
          ...
          (proximal_delta == settings.proximal_qp.minimum_regularization &&
        ...
        proximal_delta =
            std::max(proximal_regularization_limit,
                     (1.0 - proximal_regularization_progress) * proximal_delta);
      } else {
        ++dual_center_update_rejections_before_regularization_refinement;
